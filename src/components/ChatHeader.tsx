...
import { X, ShieldAlert, Radio } from "lucide-react";
  ...
  return (
    <div className="h-16 border-b border-base-100 bg-base-900/60 px-4 flex items-center justify-between backdrop-blur-sm">
      ...
      <div className="flex items-center gap-3">
        {/* Profile Avatar with Live Ping Indicator Rings */}
        <div className="relative">
          <div className="w-9 h-9 rounded-xl bg-base-800 border border-base-100 overflow-hidden">
            <img
              src={selectedChat.chatImage || "https://api.dicebear.com/7.x/bottts/svg?seed=" + selectedChat.chatName} 
              alt={selectedChat.chatName}
              className="w-full h-full object-cover"
            />
          </div>
          {/* Real-time status badge */}
          <span className={`absolute bottom-0 right-0 block h-2.5 w-2.5 rounded-full ring-2 ring-base-900
          ...
          <div className="flex items-center gap-1 mt-0.5">
            {isOnline ? (
              <>
                ...
                <span className="text-[9px] text-success uppercase tracking-wider font-bold">Online</span>
              </>
            ) : (
              <>
                <ShieldAlert className="w-2.5 h-2.5 text-base-content/30" />
                <span className="text-[9px] text-base-content/40 uppercase tracking-wider">Offline</span>
              </>
            )}
          </div>
        </div>
